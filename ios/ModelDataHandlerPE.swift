...
class ModelDataHandlerPE {
  ...
  init(
    threadCount: Int = ConstantsPE.defaultThreadCount,
    delegate: Delegates = ConstantsPE.defaultDelegate
  ) throws {
  ...
  private func postprocess(to viewSize: CGSize) -> ResultPE? {
    ...
    var result = ResultPE(dots: [], lines: [], score: totalScore)
