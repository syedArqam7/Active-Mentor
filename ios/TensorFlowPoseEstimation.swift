...
class TensorFlowPoseEstimation: UIViewController {
  // MARK: Storyboards Connections
  @IBOutlet weak var previewView: PreviewView!

  @IBOutlet weak var overlayViewPE: OverlayViewPE!
  ...
  var threadCount: Int = ConstantsPE.defaultThreadCount
  var delegate: Delegates = ConstantsPE.defaultDelegate
  ...
  private lazy var cameraCapture = CameraFeedManagerPE(previewView: previewView)

  // Handles all data preprocessing and makes calls to run inference.
  private var modelDataHandler: ModelDataHandlerPE?
  ...
  override func viewDidLoad() {
    ...
    do {
      modelDataHandler = try ModelDataHandlerPE()
    } catch let error {
    ...
    threadCountStepper.value = Double(ConstantsPE.defaultThreadCount)
    threadCountLabel.text = ConstantsPE.defaultThreadCount.description
  ...
  override func viewDidLayoutSubviews() {
    overlayViewFrame = overlayViewPE.frame
  ...
  @IBAction func didChangeThreadCount(_ sender: UIStepper) {
    ...
    do {
      modelDataHandler = try ModelDataHandlerPE(threadCount: changedCount, delegate: delegate)
    } catch let error {
      fatalError(error.localizedDescription)
    }
  ...
  @IBAction func didChangeDelegate(_ sender: UISegmentedControl) {
    ...
    do {
      modelDataHandler = try ModelDataHandlerPE(threadCount: threadCount, delegate: changedDelegate)
    } catch let error {
      fatalError(error.localizedDescription)
    }
...
extension TensorFlowPoseEstimation: CameraFeedManagerDelegatePE {
  func cameraFeedManager(_ manager: CameraFeedManagerPE, didOutput pixelBuffer: CVPixelBuffer) {
    runModel(on: pixelBuffer)
  }

  // MARK: Session Handling Alerts
  func cameraFeedManagerDidEncounterSessionRunTimeError(_ manager: CameraFeedManagerPE) {
  ...
  func cameraFeedManager(
    _ manager: CameraFeedManagerPE, sessionWasInterrupted canResumeManually: Bool
  ) {
  ...
  func cameraFeedManagerDidEndSessionInterruption(_ manager: CameraFeedManagerPE) {
    // Updates UI once session interruption has ended.
    self.cameraUnavailableLabel.isHidden = true
    self.resumeButton.isHidden = true
  }

  func presentVideoConfigurationErrorAlert(_ manager: CameraFeedManagerPE) {
  ...
  func presentCameraPermissionsDeniedAlert(_ manager: CameraFeedManagerPE) {
    let alertController = UIAlertController(
  ...
  func drawResult(of result: ResultPE) {
    self.overlayViewPE.dots = result.dots
    self.overlayViewPE.lines = result.lines
    self.overlayViewPE.setNeedsDisplay()
  }

  func clearResult() {
    self.overlayViewPE.clear()
    self.overlayViewPE.setNeedsDisplay()
  }
}

// MARK: - TableViewDelegate, TableViewDataSource Methods
extension TensorFlowPoseEstimation: UITableViewDelegate, UITableViewDataSource {
